...
- has() 选择器:“根据子元素的特征 _反向选择父元素_ , 也被称为 “父选择器” 或 “存在选择器”。
...
- 动态修改html文字大小（基准值）方案：
  ...
  ```
  设计稿转换公式：rem值=设计稿元素尺寸/html的font-size（基准值）3. 使用Postcss插件 (最佳实践) 4. 转换插件：flexible.js 实现动态修改html根字号大小。使用插件实现px转换为rem单位。
...
一种网页设计方法，网页根据***用户设备***的屏幕尺寸、分辨率、方向(横屏/竖屏)等特性，**_自动调整布局和内容_**，提供一致且优化的用户体验。其核心目标是“一次设计，多端适配”。
...
```css
mask-image: 线性渐变/径向渐变;
mask-image: url(../xx.jpg);
```
